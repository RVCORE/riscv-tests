...
// void va2pa(uintptr_t va, uintptr_t pa, int level)
// {
//   assert((uintptr_t)va % PGSIZE == 0);
//   assert((uintptr_t)pa % PGSIZE == 0);
//   assert(level >= 2);
//   uint64_t *pg_base = l1pt;
//   uint64_t *pte;
//   int index = level;
//   while (index--) {
//     uint32_t pte_index = ((va >> PGSIZE) >> (index*RISCV_PGLEVEL_BITS)) & 0x1ff;
//     pte = &pg_base[pte_index];
//     pg_base = (uint64_t *)PTE_ADDR(*pte);
//     if (index == 0) break;
//     if (!(*pte & PTE_V)) {
//       pg_base = allocate_page();
//       *pte = ((uintptr_t)pg_base >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
//     }
//   }

//   if (!(*pte & PTE_V)) {
//     *pte = PTE_V | ((uintptr_t)pa >> PGSHIFT << PTE_PPN_SHIFT);
//   }
// }
...
{
#ifdef RVTEST_AM
  printf("[vm_boot] Address Translation Scheme %s\n", SATP_MODE_CHOICE == 1 ? "Sv32" : 
        (SATP_MODE_CHOICE == 8  ? "Sv39" : 
        (SATP_MODE_CHOICE == 9  ? "Sv48" : 
        (SATP_MODE_CHOICE == 10 ? "Sv57" : 
        (SATP_MODE_CHOICE == 11 ? "Sv64" : "Sv??")))));
#endif
  uint64_t random = ENTROPY;
